...
export function useConfig() {
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      // FIX: With fetchConfig now returning the correct shape, this assignment is valid.
      setConfig(data);
    } catch (err) {
...
export function useProgress() {
    ...
    const loadProgress = useCallback(async () => {
        ...
        try {
            ...
            // FIX: Ensure data from the service is correctly typed as ProgressRecord.
            // The service now handles this, but an explicit cast/map here adds robustness.
